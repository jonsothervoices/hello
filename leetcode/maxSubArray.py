...
def maxSubArray(nums):
    ...
    l = len(nums)
    # if l > 50:
    #     return doTheThing(nums)
    # else:
    return doTheOtherThing(nums)
...
#     else:
#         mid = l//2
#     lo = doTheThing(nums[:mid])
#     hi = doTheThing(nums[mid:])
#     return max(max(lo,hi),mySum)

def doTheOtherThing(nums):
    l = len(nums)
    mySum = 0
    mid = 0
    if l == 1:
    ...
    for i in range (l):
        if i == 0:
            continue
        if nums[i] + myMax > myMax:
            myMax += nums[i]
    lo = doTheOtherThing(nums[1:])
    hi = doTheOtherThing(nums[:l-1])
    return max(max(lo,hi),mySum)
